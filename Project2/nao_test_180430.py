...
def plotRobotMove(x, y, theta=0, label='b+'):
    ...
    plt.plot(x, y-CELL_DIMENSION/2, label)
    plt.draw()
    plt.pause(0.01)
    plt.show()

def plotWallFromLandmark(x, y, label='go-', linewidth=1, landmarkLocation='north'):

    print "WALL x="+str(x)+", y="+str(y)
    if (landmarkLocation=='west') :
        # west
        start = x-CELL_DIMENSION/2, y
        end = x-CELL_DIMENSION/2, y-CELL_DIMENSION
    elif (landmarkLocation=='east') :
        # east
        start = x+CELL_DIMENSION/2, y
        end = x+CELL_DIMENSION/2, y-CELL_DIMENSION
    elif (landmarkLocation=='south') :
        # south
...
if (abs(np.median(left_y)) > 0 and abs(np.median(left_y)) <= CELL_DIMENSION):
    plotWallFromLandmark(np.median(left_x)+(CELL_DIMENSION/2), np.median(left_y)-(CELL_DIMENSION/2),label='go-',linewidth=20, landmarkLocation='south')
...
if (abs(np.median(right_y)) > 0 and abs(np.median(right_y)) <= CELL_DIMENSION):
    plotWallFromLandmark(np.median(right_x)+(CELL_DIMENSION/2), np.median(right_y)+(CELL_DIMENSION/2),label='go-',linewidth=20, landmarkLocation='north')
